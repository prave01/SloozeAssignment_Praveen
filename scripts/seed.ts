...
async function seed() {
    ...
    try {
        ...
        if (indiaRest) {
            await db.insert(menu).values({ id: v7(), restaurantId: indiaRest.id }).onConflictDoNothing();
        }
        if (americaRest) {
            await db.insert(menu).values({ id: v7(), restaurantId: americaRest.id }).onConflictDoNothing();
        }
        ...
        for (const u of usersToCreate) {
            ...
            if (res.user) {
                ...
                if (u.location === "america" && americaRest) restaurantID = americaRest.id;
